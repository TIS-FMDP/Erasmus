...
$stupen_body = ($submit) ? $safe->input($_POST['stupen_body']) : 0;
$jazyk_body = ($submit) ? $safe->input($_POST['jazyk_body']) : 0;
$aktivity_body = ($submit) ? $safe->input($_POST['aktivity_body']) : 0;
  ...
  $date_to_ = $date_to[2].'/'.$date_to[1].'/'.$date_to[0];
...
$sql1 = 'UPDATE STUDENTS
    SET FIRSTNAME="'. $student_name .'", LASTNAME="'. $student_surname .'",BORN="'. $born .'",GENDER="'. $gender .'",CITIZENSHIP="'. $citizenship .'", YEAR="'. $year .'"
    WHERE id="'. $id_stud .'"';
...
if($userrole === "admin"){
    ...
    $sql ='UPDATE STUDENT_EXCHANGES
        SET STUDY_YEAR="'. $study_year .'",SEMESTER="'. $semester .'",SOCIALSTIPEND="'. $soc .'",HANDICAPPED="'. $ztp .'",NOTES="'. $notes .'",YEAR="'. $year .'",ADDRESS="'. $address .'",PHONE="'. $phone .'",STUDENT_YEAR="'. $student_year .'"
        WHERE id="'. $id_exchange .'"';
    $query2 = mysqli_query($link,$sql) or die(mysqli_error($link));
}
if($userrole === "admin"){
    ...
    $sql ='UPDATE STUDY_POINTS
        SET POINTS="'. $stupen_body .'"
        WHERE ID_STUDENT="'. $id_stud .'" AND TYPE = 1';
    $query6 = mysqli_query($link,$sql) or die(mysqli_error($link));
    
    $sql ='UPDATE STUDY_POINTS
        SET POINTS="'. $jazyk_body .'"
        WHERE ID_STUDENT="'. $id_stud .'" AND TYPE = 2';
    $query7 = mysqli_query($link,$sql) or die(mysqli_error($link));
    
    $sql ='UPDATE STUDY_POINTS
        SET POINTS="'. $aktivity_body .'"
        WHERE ID_STUDENT="'. $id_stud .'" AND TYPE = 3';
    $query8 = mysqli_query($link,$sql) or die(mysqli_error($link));
...
<div class="form-group">
  ...
  <div class="col-md-1">
  <input id="textinput" name="stupen_body" type="text" value="<?=$points_array[0]?>"  class="form-control input-md">  
  </div>
</div>
...
<div class="form-group">
  ...
  <div class="col-md-1">
  <input id="textinput" name="jazyk_body" type="text" value="<?=$points_array[1]?>"  class="form-control input-md">  
  </div>
</div>
...
<div class="form-group">
  ...
  <div class="col-md-1">
  <input id="textinput" name="aktivity_body" type="text" value="<?=$points_array[2]?>"  class="form-control input-md">  
  </div>
</div>
<?php
}
